...
def getClusterFromKMeans(titles, num_clusters):
    word_lists = [vectorize_text(title) for title in titles]
    word_set = make_word_set(word_lists)
    word_vectors = make_word_vectors(word_set, word_lists)

    titles_map = dict(zip(map(str, make_word_vectors(word_set, word_lists)), titles))

    k_means = KMeans(num_clusters, list(word_vectors))
    k_means.main_loop()
    return translator(k_means.clusters, titles_map)
